...
class Users extends Public_Controller {
    ...
    public function view($username = null) {
        ...
        $this->template
                ...
                ->set('images_publicate', $images_publicate)
                ->set('profile', $profiles[0])
                ->build('profile/view', array(
                    '_user' => $user,
        ));
    }
    ...
    public function sessionApp() {
        ...
        if ($this->form_validation->run() or $this->current_user) {
            ...
            exit(json_encode(array('status' => false, 'message' => 'El usuario ingresado es incorrecto')));
        }
    }
        ...
        if ($_POST) {
            if ($this->form_validation->run()) {
                ...
                if ($id > 0) {
                    ...
                    elseif ((int) Settings::get('activation_email') === 2) {
                        $this->ion_auth->activate($id, false);
                        $users = $this->ion_auth->login($this->input->post('email'), $this->input->post('password'));
                        echo json_encode($users);
                    } else {
                        $this->ion_auth->deactivate($id);

                        /* show that admin needs to activate your account */
                        $users = array('msg' => lang('user:activation_by_admin_notice'));
                        echo json_encode($users); /* bump it to show the flash data */
                    }
                }

                // Can't create the user, show why
                else {
                    echo json_encode($users = array('msg' => $this->ion_auth->errors()));
                }
            } else {
                // Return the validation error
                echo json_encode($users = array('msg' => $this->form_validation->error_string()));
            }
        }
        // Is there a user hash?
        else {
            echo json_encode($users = array('msg' => 'Ningun dato enviado'));
        }
        // --------------------------------
        // Create profile fields.
        // --------------------------------
    ...
    public function resetPassapp($code = null) {
        ...
        if ($this->input->post('email')) {
            ...
            if ($user_meta) {
                ...
                if ($new_password) {
                    //set success message
                    echo json_encode($users = array('msg' => lang('forgot_password_successful')));
                } else {
                    // Set an error message explaining the reset failed
                    echo json_encode($users = array('msg' => $this->ion_auth->errors()));
                }
                ...
                //wrong username / email combination
                echo json_encode($users = array('msg' => lang('user:forgot_incorrect')));
            }
        }
    }
